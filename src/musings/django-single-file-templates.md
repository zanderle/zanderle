...
description: Django Single File Templates -- inspired by Vue's Single file components. Make your Django templates more organized by holding your HTML and all the related CSS and JavaScript in a sensible way.
permalink: /musings/2020/11/django-single-file-templates/
---
...
1) Let's create our template `page.html`. We'll probably want to extend from an existing base, so let's use `{% raw %}{% extends %}{% endraw %}`
...
{% block scripts %}<script src="{% static 'app/page.js' %}"></script>{% endblock %}{% endraw %}
```
